...
describe('Sitemap', () => {
    ...
    it('should have correct metadata for each route', () => {
        ...
        const homePage = sitemapData.find(entry => entry.url === 'https://learn.fx64b.dev')
        expect(homePage?.priority).toBe(1)
        expect(homePage?.changeFrequency).toBe('daily')
        
        // Check pricing page
        const pricingPage = sitemapData.find(entry => entry.url === 'https://learn.fx64b.dev/pricing')
        expect(pricingPage?.priority).toBe(0.8)
        expect(pricingPage?.changeFrequency).toBe('weekly')
        
        // Check legal pages have lower priority
        const termsPage = sitemapData.find(entry => entry.url === 'https://learn.fx64b.dev/terms')
