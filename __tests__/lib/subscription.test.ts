...
describe('Subscription Service', () => {
  ...
  describe('getUserSubscription', () => {
    test('should return subscription for valid user ID', async () => {
      ...
      expect(mockWhere).toHaveBeenCalledWith({ field: subscriptions.userId, value: 'user-1', type: 'eq' })
    ...
    test('should throw error for invalid user ID', async () => {
      await expect(getUserSubscription('')).rejects.toThrow('Invalid user ID provided')
      await expect(getUserSubscription(null as any)).rejects.toThrow('Invalid user ID provided')
      await expect(getUserSubscription(undefined as any)).rejects.toThrow('Invalid user ID provided')
      await expect(getUserSubscription(123 as any)).rejects.toThrow('Invalid user ID provided')
    })
    ...
    test('should handle database errors', async () => {
      ...
      await expect(getUserSubscription('user-1')).rejects.toThrow('Failed to fetch subscription data')
    })
  ...
  describe('getCachedUserSubscription', () => {
    test('should throw error for missing user ID', () => {
      expect(() => getCachedUserSubscription('')).toThrow('User ID is required for cached subscription lookup')
      expect(() => getCachedUserSubscription(null as any)).toThrow('User ID is required for cached subscription lookup')
      expect(() => getCachedUserSubscription(undefined as any)).toThrow('User ID is required for cached subscription lookup')
    })
