...
describe('Preferences Actions', () => {
  ...
  describe('getUserPreferences', () => {
    test('should return user preferences when found', async () => {
      ...
      expect(mockWhere).toHaveBeenCalledWith({ field: userPreferences.userId, value: 'user-1', type: 'eq' })
  ...
  describe('updateUserPreferences', () => {
    ...
    test('should update existing preferences successfully', async () => {
      ...
      const mockExistingPreferences = [{ userId: 'user-1', theme: 'dark' }]
      const mockSelectLimit = vi.fn().mockResolvedValue(mockExistingPreferences)
    ...
    test('should handle database errors during update', async () => {
      ...
      const mockSelectLimit = vi.fn().mockRejectedValue(new Error('Database error'))
      const mockSelectWhere = vi.fn().mockReturnValue({ limit: mockSelectLimit })
      const mockSelectFrom = vi.fn().mockReturnValue({ where: mockSelectWhere })
      const mockSelect = vi.fn().mockReturnValue({ from: mockSelectFrom })
