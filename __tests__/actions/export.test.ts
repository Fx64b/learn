...
const mockGetFlashcardsByDeckId = vi.mocked(flashcardActions.getFlashcardsByDeckId)
...
describe('Export Actions', () => {
  ...
  describe('getExportableFlashcards', () => {
    ...
    test('should handle invalid deck id', async () => {
      ...
      expect(result).toEqual([])
      expect(mockGetFlashcardsByDeckId).toHaveBeenCalledWith('invalid-deck')
    })
    ...
    test('should propagate errors from getFlashcardsByDeckId', async () => {
      ...
      await expect(getExportableFlashcards('deck-1')).rejects.toThrow('Database connection failed')
    })
  })
})